...
import { updateAircraft, Aircraft } from "@/services/aircraft.service";
...
export default function EditAircraftModal({ isOpen, onClose, onSuccess, aircraft }: EditAircraftModalProps) {
  ...
  useEffect(() => {
    if (aircraft) {
      setFormData({
        ...
        economySeatCount: aircraft.economySeatCount?.toString() || "",
        businessSeatCount: aircraft.businessSeatCount?.toString() || "",
        firstSeatCount: aircraft.firstSeatCount?.toString() || ""
      });
    }
  }, [aircraft]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm() || !aircraft) return;
    ...
    try {
      ...
      setGeneralError(error.response?.data?.message || "An error occurred while updating the aircraft");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col border border-slate-200">
        <div className="p-8 border-b border-slate-200 flex-shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Edit Aircraft</h2>
              <p className="text-slate-600 mt-1">Update aircraft model and seating configuration</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6" id="editAircraftForm">
